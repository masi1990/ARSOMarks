...
export class Operator {
  ...
  @Column({ name: 'operator_type', type: 'enum', enum: OperatorType })
  operatorType: OperatorType;

  @Column({ name: 'company_legal_name', length: 200 })
  companyLegalName: string;

  @Column({ name: 'trading_name', length: 150, nullable: true })
  tradingName?: string;

  @Column({ name: 'registration_number_business', length: 50 })
  ...
  @Column({ name: 'year_established', type: 'int' })
  yearEstablished: number;

  @Column({ name: 'company_age', type: 'int', nullable: true })
  companyAge?: number; // Auto-calculated

  @Column({ name: 'legal_structure', type: 'enum', enum: LegalStructure })
  legalStructure: LegalStructure;

  @Column({ name: 'business_activity', type: 'text' })
  businessActivity: string;

  // A2: Company Size & Financial Information
  @Column({ name: 'employee_count', type: 'enum', enum: EmployeeCountRange })
  employeeCount: EmployeeCountRange;

  @Column({ name: 'annual_turnover', type: 'enum', enum: AnnualTurnoverRange })
  ...
  @Column({ name: 'women_owned', type: 'enum', enum: OwnershipStatus })
  ...
  @Column({ name: 'beneficial_owners_count', type: 'int' })
  beneficialOwnersCount: number;

  @Column({ name: 'pep_involved', type: 'boolean', default: false })
  pepInvolved: boolean;
