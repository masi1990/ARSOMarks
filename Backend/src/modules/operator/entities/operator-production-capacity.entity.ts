...
export class OperatorProductionCapacity {
  ...
  @Column({ name: 'production_capacity', type: 'decimal', precision: 15, scale: 2, nullable: true })
  productionCapacity?: number;

  @Column({ name: 'capacity_unit', length: 20, nullable: true })
  capacityUnit?: string;

  @Column({ name: 'capacity_utilization', type: 'decimal', precision: 5, scale: 2, nullable: true })
  capacityUtilization?: number;

  @Column({ name: 'quality_management', length: 20, nullable: true })
  qualityManagement?: string; // 'YES', 'NO', 'IN_PROGRESS'

  @Column({ name: 'qms_type', type: 'enum', enum: QMSType, nullable: true })
  qmsType?: QMSType;

  @Column({ name: 'certification_count', type: 'int', nullable: true })
  certificationCount?: number;

  @Column({ name: 'existing_certifications', type: 'text', nullable: true })
  existingCertifications?: string;

  @Column({ name: 'technical_staff', type: 'int', nullable: true })
