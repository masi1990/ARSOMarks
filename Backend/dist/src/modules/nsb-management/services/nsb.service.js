...
let NsbService = class NsbService {
    constructor(nsbRepository, contactRepository, locationRepository, documentRepository, dataSource) {
    ...
    async findByUser(user) {
        if (user.organizationId) {
            const nsb = await this.nsbRepository.findOne({
                where: { id: user.organizationId, status: enums_1.NsbStatus.ACTIVE },
                relations: ['contacts', 'locations', 'documents', 'country', 'country.region', 'country.recMemberships', 'country.recMemberships.rec'],
            });
            if (nsb) {
                return nsb;
        ...
        if (user.countryId) {
            const nsb = await this.nsbRepository.findOne({
                where: { countryId: user.countryId, status: enums_1.NsbStatus.ACTIVE },
                relations: ['contacts', 'locations', 'documents', 'country', 'country.region', 'country.recMemberships', 'country.recMemberships.rec'],
            });
            if (nsb) {
                return nsb;
...
exports.NsbService = NsbService = __decorate([
    ...
    __param(3, (0, typeorm_1.InjectRepository)(nsb_document_entity_1.NsbDocument)),
